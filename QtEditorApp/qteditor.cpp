...
void QtEditor::saveFile()
{
    ...
    QString fileName = filePathMap.value(textedit);

    if (fileName.isEmpty()) {
        saveAsFile(); // 파일명이 없으면 Save As로 대체
        return;
    }
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        QMessageBox::warning(this, "Save File", "Cannot save file: " + file.errorString());
    }
}
...
void QtEditor::saveAsFile()
{
    ...
    if (!fileName.isEmpty()) {
        ...
        if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            ...
            filePathMap[textedit] = fileName;
            subWindow->setWindowTitle(QFileInfo(fileName).fileName());
            statusBar()->showMessage("File saved as: " + fileName);
        } else {
            QMessageBox::warning(this, "Save File As", "Cannot save file: " + file.errorString());
        }
    }
}
